    ...
    if not pyfftw_exists:
        if len(a.shape) == 4:
            return numpy.fft.fftshift(
                numpy.fft.fft2(numpy.fft.ifftshift(a, axes=[2, 3])),
                axes=[2, 3],
            )
        if len(a.shape) == 5:
            return numpy.fft.fftshift(
                numpy.fft.fft2(numpy.fft.ifftshift(a, axes=[3, 4])),
                axes=[3, 4],
            )
...
def ifft(a):
    ...
    if not pyfftw_exists:
        if len(a.shape) == 4:
            return numpy.fft.fftshift(
                numpy.fft.ifft2(numpy.fft.ifftshift(a, axes=[2, 3])),
                axes=[2, 3],
            )
        if len(a.shape) == 5:
            return numpy.fft.fftshift(
                numpy.fft.ifft2(numpy.fft.ifftshift(a, axes=[2, 3, 4])),
                axes=[2, 3, 4],
            )
...
def extract_oversampled(a, xf, yf, kernel_oversampling, kernelwidth):
    ...
    assert mx >= 0 and my >= 0, f"mx {mx} and my {my}"
